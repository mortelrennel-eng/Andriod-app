...
public class AddStudentsToSectionActivity extends AppCompatActivity {
    ...
    private ArrayList<String> studentDisplayList; // For ListView display
    private ArrayList<User> allUnassignedStudents; // Holds the full user objects
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        studentsListView.setOnItemClickListener((parent, view, position, id) -> {
            // This logic is now handled by the adapter and filter
        });
    ...
    private void addSelectedStudentsToSection() {
        ...
        for (int i = 0; i < studentsListView.getCount(); i++) {
            if (studentsListView.isItemChecked(i)) {
                String name = (String) studentsListView.getItemAtPosition(i);
